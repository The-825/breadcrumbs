...
function repository(item, all, researchItems, claimsById) {
  ...
  const unknown = Object.entries(item.mechanisms).filter(([, value]) => value === "U").map(([key]) => key.replaceAll("_", " "));
  const notObserved = item.mechanismTotal - visible.length - partial.length - unknown.length;
  const popularity = item.popularityOrder ? `${item.popularityOrder} / ${item.detailedReviewCount}` : "Not collected";
  const categoryRank = item.categoryPopularityOrder ? `${item.categoryPopularityOrder} / ${item.categoryRepositoryCount}` : "Not collected";
  const stars = item.stars_observed === null ? "Not collected" : Number(item.stars_observed).toLocaleString();
  return `<p class="eyebrow">Repository profile</p><h1>${esc(item.repository)}</h1><p class="lede">${esc(item.evidence_note)}</p>
    <div class="profile"><div class="meter"><small>Global popularity</small><strong>${popularity}</strong></div><div class="meter"><small>Within category</small><strong>${categoryRank}</strong><span>${esc(item.category.replaceAll("-", " "))}</span></div><div class="meter"><small>Stars observed</small><strong>${stars}</strong><span>${item.stars_observed === null ? "" : `on ${esc(item.starsObservedDate)}`}</span></div><div class="meter"><small>Last reviewed</small><strong>${esc(item.lastReviewed)}</strong><span>${esc(item.evidence_depth.replaceAll("-", " "))}</span></div></div>
    <section class="detail split-detail"><div><p class="eyebrow">Our reading</p><h2>What is visible</h2><p>${esc(sentence(item.evidence_note))}</p><h3>Classification</h3><p><strong>Category:</strong> ${esc(item.category.replaceAll("-", " "))}<br><strong>Selection lens:</strong> ${esc(item.selection_aspect.replaceAll("-", " "))}</p><h3>Linked claims</h3><div class="claim-links">${claimLinks(item.claims, claimsById)}</div></div><div class="signal-panel"><h3>Mechanism profile</h3>${bar("Visible", visible.length, item.mechanismTotal)}${bar("Partial", partial.length, item.mechanismTotal, "partial")}${bar("Unknown", unknown.length, item.mechanismTotal, "quiet")}${bar("Not observed or out of scope", notObserved, item.mechanismTotal, "quiet")}<p><strong>Visible</strong> means the pinned evidence directly exposed the mechanism. <strong>Partial</strong> means only part of the mechanism was exposed or the evidence was incomplete. <strong>Unknown</strong> means the portable assessment did not establish the mechanism.</p><p class="note">This chart reports what the recorded evidence supports. It is not a performance score.</p></div></section>
