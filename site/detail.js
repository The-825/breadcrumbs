...
function research(item, all, repositories) {
  const related = all.filter(candidate => candidate.id !== item.id && candidate.claims.some(claim => item.claims.includes(claim))).slice(0, 8);
  const linked = repositories.filter(candidate => candidate.claims.some(claim => item.claims.includes(claim))).slice(0, 8);
  return `<p class="eyebrow">Research profile</p><h1>${esc(item.title)}</h1><p class="lede">${esc(item.finding)}</p>
    ...
    <section class="detail split-detail"><div><p class="eyebrow">Our reading</p><h2>What this changes</h2><p>${esc(item.impact)}</p><h3>Boundary and next question</h3><p>${esc(item.boundary)}</p></div><div class="signal-panel"><h3>Evidence profile</h3>${bar("Directness", item.directnessValue, 3)}${bar("Horizon", item.horizonValue, 3, "partial")}<p><strong>Family:</strong> ${esc(item.family)}</p><p><strong>Design:</strong> ${esc(item.design)}</p></div></section>
    <section class="section related"><div class="card"><h2>Visible risks</h2><p>${esc(item.flags)}</p><h3>Linked claims</h3>${pills(item.claims)}</div><div class="card"><h2>Source</h2><p>The original source remains authoritative. This profile is a bounded interpretation.</p><a class="button" href="${esc(item.url)}">Open original source</a></div></section>
...
function repository(item, all, researchItems) {
  const related = all.filter(candidate => candidate.id !== item.id && (candidate.selection_aspect === item.selection_aspect || candidate.claims.some(claim => item.claims.includes(claim)))).slice(0, 8);
  const linked = researchItems.filter(candidate => candidate.claims.some(claim => item.claims.includes(claim))).slice(0, 8);
  ...
  return `<p class="eyebrow">Repository profile</p><h1>${esc(item.repository)}</h1><p class="lede">${esc(item.evidence_note)}</p>
    <div class="profile"><div class="meter"><small>Global popularity</small><strong>${item.popularityOrder} / 100</strong></div><div class="meter"><small>Within ${esc(item.selection_aspect)}</small><strong>${item.aspectPopularityOrder} / ${item.aspectRepositoryCount}</strong></div><div class="meter"><small>Stars observed</small><strong>${Number(item.stars_observed).toLocaleString()}</strong><span>on ${esc(item.starsObservedDate)}</span></div><div class="meter"><small>Last reviewed</small><strong>${esc(item.lastReviewed)}</strong><span>${esc(item.evidence_depth)}</span></div></div>
    <section class="detail split-detail"><div><p class="eyebrow">Our reading</p><h2>What is visible</h2><p>${esc(item.evidence_note)}</p><h3>Primary mechanism area</h3><p>${esc(item.selection_aspect)} within ${esc(item.category)}.</p><h3>Linked claims</h3>${pills(item.claims)}</div><div class="signal-panel"><h3>Mechanism profile</h3>${bar("Visible", visible.length, item.mechanismTotal)}${bar("Partial", partial.length, item.mechanismTotal, "partial")}${bar("Not observed or out of scope", notObserved, item.mechanismTotal, "quiet")}<p class="note">This chart reports what the pinned review found. It is not a performance score.</p></div></section>
...
async function init() {
  ...
  const [current, researchItems, repositories] = await Promise.all([fetch(`data/${type}.json`).then(response => response.json()), fetch("data/research.json").then(response => response.json()), fetch("data/repositories.json").then(response => response.json())]);
  const item = current.find(candidate => candidate.id === id);
  if (!item) throw Error();
  document.title = `${item.title || item.repository} | Breadcrumbs`;
  document.querySelector("main").innerHTML = type === "research" ? research(item, researchItems, repositories) : repository(item, repositories, researchItems);
}
