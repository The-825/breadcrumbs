...
| [memory_engine.py](memory_engine.py) | Three-tier runnable memory (working state, append-only episodes, semantic facts with asserted-vs-verified status): compaction flushes down instead of deleting, verification refuses an empty oracle, rejected values leave tombstones that block silent re-assertion, retrieval is deterministic (as-of replay on the learned-at axis, valid-at windows on world time, audience scoping that makes the publication boundary mechanical) and names its own limits |
