...
| [governed_replay.py](governed_replay.py) | Offline replay sampler and typed proposal queue: correction signals, exact recurrence, and recency select episodes deterministically; an external evaluator may propose a fact, skill, watch, or correction, but failure stays unknown and every proposal is source-linked, idempotent, pending review, and non-mutating |
