...
| [workflows/](workflows/) | The fail-closed automerge gate (`automerge.yml`) with its extracted decision script and tests, the merge-lane companion workflows (`greenlight-all.yml`, `greenlight-command.yml`), and two pattern docs: [AUTOMERGE_GOTCHAS.md](workflows/AUTOMERGE_GOTCHAS.md) and [MERGE_LANE_COMPANIONS.md](workflows/MERGE_LANE_COMPANIONS.md). |
| [`.github/workflows/sync-ci-kit.yml`](../.github/workflows/sync-ci-kit.yml) | Reusable `workflow_call` that keeps a downstream repo's copy of the two greenlight workflows in step with the templates above. A one-time copy-paste adoption drifts silently after that (the same bug got fixed independently in six different repos before this existed); a thin per-repo wrapper calling this on a schedule turns future template fixes into a PR instead of a rediscovery. Not specific to this repo: any repo can call it. |
| [preflight/](preflight/README.md) | Catches the branch collision BEFORE the push: whether the branch is behind its base, whether another open branch touches the same files (the stacked-PR problem GitHub does not solve), placeholder commit subjects, and repeated base merges. A check that cannot run reports SKIPPED, never PASSED. 19 self-tests. |
